        ...
        <form action="{{ route('notifications.store') }}" method="POST" class="bg-white shadow-md rounded p-6">
            ...
            <div class="mb-4">
                ...
                <select name="target_group" id="target_group" class="w-full border border-gray-300 rounded px-3 py-2" required>
                    <option value="Student" {{ old('target_group') == 'Student' ? 'selected' : '' }}>Student</option>
                    <option value="Instructor" {{ old('target_group') == 'Instructor' ? 'selected' : '' }}>Instructor</option>
            ...
            <div class="mb-4">
                ...
                <select name="notification_type" id="notification_type" class="w-full border border-gray-300 rounded px-3 py-2" required>
                    <option value="Sick" {{ old('notification_type') == 'Sick' ? 'selected' : '' }}>Ziek</option>
                    <option value="LessonChange" {{ old('notification_type') == 'LessonChange' ? 'selected' : '' }}>Les Wijziging</option>
                    <option value="LessonCancel" {{ old('notification_type') == 'LessonCancel' ? 'selected' : '' }}>Les Geannuleerd</option>
                    <option value="PickupAddressChange" {{ old('notification_type') == 'PickupAddressChange' ? 'selected' : '' }}>Ophaaladres Wijziging</option>
                    <option value="LessonGoalChange" {{ old('notification_type') == 'LessonGoalChange' ? 'selected' : '' }}>Les Doel Wijziging</option>
                </select>
            ...
            <div class="mb-4">
                ...
                <input type="date" name="date" id="date" value="{{ old('date', date('Y-m-d')) }}" required class="w-full border border-gray-300 rounded px-3 py-2">
            ...
            <div class="mb-4">
                ...
                <textarea name="note" id="note" rows="2" class="w-full border border-gray-300 rounded px-3 py-2">{{ old('note') }}</textarea>
            </div>
