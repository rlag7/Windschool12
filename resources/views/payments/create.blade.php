...
@section('content')
    <div class="container mx-auto mt-8 px-6 max-w-lg">
        <h1 class="text-3xl font-bold mb-6 text-[#002E5B] flex items-center gap-3">
            <i class="fas fa-plus-circle"></i> Nieuwe Betaling Aanmaken
        </h1>

        <form action="{{ route('payments.store') }}" method="POST" class="bg-white shadow-md rounded p-6">
            @csrf
            ...
            <div class="mb-4">
                ...
                <select name="status" id="status" required
                        class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#002E5B]">
                    <option value="In Afwachting" {{ old('status') == 'In Afwachting' ? 'selected' : '' }}>In Afwachting</option>
            ...
            <div class="mb-6">
                ...
                <textarea name="note" id="note" rows="3" class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#002E5B]">{{ old('note') }}</textarea>
            </div>
